...
public class CloudWorker1 implements Runnable {
	...
	private void compute(String input[]) {
		...
		for (int i = 0; i < input.length; i++) {
			...
			if (input[i].equals("+")) {
				int tempPlus;
				tempPlus=CloudWorker.getPlus();
				tempPlus++;
				CloudWorker.setPlus(tempPlus);
				...
				int tempMinus;
				tempMinus=CloudWorker.getMinus();
				tempMinus++;
				CloudWorker.setMinus(tempMinus);
				temp = sendCalculation(nodeLookUp("-"),
						Arrays.copyOfRange(input, i - 1, i + 2));
				input[i + 1] = temp;
				credits += 50;
				...
				int tempMult;
				tempMult=CloudWorker.getMult();
				tempMult++;
				CloudWorker.setMult(tempMult);
				temp = sendCalculation(nodeLookUp("*"),
						Arrays.copyOfRange(input, i - 1, i + 2));
				input[i + 1] = temp;
				credits += 50;
				...
				int tempDiv;
				tempDiv=CloudWorker.getDiv();
				tempDiv++;
				CloudWorker.setDiv(tempDiv);
				temp = sendCalculation(nodeLookUp("/"),
						Arrays.copyOfRange(input, i - 1, i + 2));
				input[i + 1] = temp;
				credits += 50;
	...
	public static void setMinus(int minus) {
		CloudWorker.minus = minus;
	}
	...
	public static void setDiv(int div) {
		CloudWorker.div = div;
	}
}
